...
def _metrics(stabilization: float, repo_kind: str, revert_rate: float = 0.1):
...
def _report_text(org: OrgResult) -> str:
    out = tempfile.mkdtemp()
    path = write_org_report(org, out_dir=out, days=90, recent_days=30)
    return Path(path).read_text()


def _metrics_json(org: OrgResult) -> dict:
    out = tempfile.mkdtemp()
    path = write_org_metrics(org, out_dir=out, days=90)
    return json.loads(Path(path).read_text())


def test_median_ignores_non_code_repo() -> None:
    text = _report_text(_org(_SERVICE, _DOCS))
    assert "60.0%" in text, "median should be the service's, not the blend"
    assert "80.0%" not in text


def test_median_json_ignores_non_code_repo() -> None:
    data = _metrics_json(_org(_SERVICE, _DOCS))
...
def test_unknown_repo_kind_counts_as_code() -> None:
    ...
    data = _metrics_json(_org(_SERVICE, legacy))
