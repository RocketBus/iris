...
interface PayloadInsights {
  ...
  stabilizationByOrigin?: Record<string, { stabilization_ratio: number; files_touched: number }>;
  cascadeRate?: number;
  cascadeMedianDepth?: number;
  cascadeByOrigin?: Record<string, { cascade_rate: number; cascades: number; total_commits: number }>;
  durabilityByOrigin?: Record<string, { survival_rate: number; lines_introduced: number; lines_surviving: number; median_age_days: number }>;
...
function DistributionBar({ data, labels, colors }: {
        ...
        {order.map((key) => {
          ...
          return (
            <div key={key} className="flex items-center gap-1.5">
              <div className={cn('size-2 rounded-full', colors[key] ?? 'bg-muted')} />
              <span>{labels[key] ?? key} {pct}%</span>
            </div>
          );
        })}
      </div>
...
function ComparisonChart({
  ...
  const hasData = data.some(
    (p) => p[humanKey] !== null || p[aiKey] !== null
  );
...
function CommitMixChart({
  ...
  const hasData = data.some((p) => p.commits_human !== null || p.commits_ai !== null);
  if (!hasData) return null;
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={200}>
          <AreaChart data={data}>
            ...
            />
            <Legend
              formatter={(value) => (value === 'commits_human' ? humanLabel : aiLabel)}
              wrapperStyle={{ fontSize: 12 }}
            />
            <Area
              type="monotone"
...
export function RepoCharts({ timeSeries, insights, aiImpact }: RepoChartsProps) {
  ...
  const hasAI = timeSeries.some(
    (p) => p.ai_detection_coverage_pct != null && p.ai_detection_coverage_pct > 0
  );
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t('repoCharts.stabilization.subtitle')}</CardDescription>
        </CardHeader>
        <CardContent>
          <MetricLineChart
            data={chartData}
      ...
      <div className={cn("grid gap-4", insights.intentDistribution && insights.originDistribution ? "md:grid-cols-2" : "md:grid-cols-1")}>
        {insights.intentDistribution && (
          <Card>
            <CardHeader>
              <CardTitle>{t('repoCharts.intent.title')}</CardTitle>
              <CardDescription>{t('repoCharts.intent.subtitle')}</CardDescription>
            </CardHeader>
            <CardContent>
              <DistributionBar
                ...
                {Object.entries(insights.intentDistribution)
                  ...
                  .map(([key, value]) => (
                    <div key={key} className="flex justify-between text-sm">
                      <span className="text-muted-foreground">{intentLabels[key] ?? key}</span>
        ...
        {insights.originDistribution && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t('repoCharts.origin.subtitle')}</CardDescription>
            </CardHeader>
            <CardContent>
              <DistributionBar
                data={insights.originDistribution}
                labels={originLabels}
                colors={{ HUMAN: 'bg-muted-foreground', AI_ASSISTED: 'bg-primary', BOT: 'bg-signal-gray' }}
              />
              {insights.stabilizationByOrigin && (
                <div className="mt-3">
                  <p className="mb-1 text-xs text-muted-foreground">{t('repoCharts.origin.stabByOrigin')}</p>
                  <div className="space-y-1">
                    {Object.entries(insights.stabilizationByOrigin).map(([origin, data]) => (
                      <div key={origin} className="flex justify-between text-sm">
                        <span className="text-muted-foreground">{originLabels[origin] ?? origin}</span>
                        <span className="font-mono">{(data.stabilization_ratio * 100).toFixed(0)}%</span>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {hasAI ? (
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t('repoCharts.aiAdoption.subtitle')}</CardDescription>
            </CardHeader>
            <CardContent>
              <MetricLineChart
                data={chartData}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t('repoCharts.commits.subtitle')}</CardDescription>
            </CardHeader>
            <CardContent>
              <MetricLineChart
                data={chartData}
      ...
      {(insights.cascadeRate != null || insights.durabilityByOrigin) && (
        <div className={cn("grid gap-4", insights.cascadeRate != null && insights.durabilityByOrigin ? "md:grid-cols-2" : "md:grid-cols-1")}>
          {insights.cascadeRate != null && (
            <Card>
              <CardHeader>
                <CardTitle>{t('repoCharts.cascades.title')}</CardTitle>
                <CardDescription>{t('repoCharts.cascades.subtitle')}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">{t('repoCharts.cascades.rate')}</span>
                  <span className="font-mono">{(insights.cascadeRate * 100).toFixed(0)}%</span>
                </div>
                {insights.cascadeMedianDepth != null && (
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">{t('repoCharts.cascades.medianDepth')}</span>
                ...
                {insights.cascadeByOrigin && Object.entries(insights.cascadeByOrigin).map(([origin, data]) => (
                  <div key={origin} className="flex justify-between text-sm">
                    <span className="text-muted-foreground">{originLabels[origin] ?? origin}</span>
                    <span className="font-mono">
          ...
          {insights.durabilityByOrigin && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>{t('repoCharts.durability.subtitle')}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-2">
                {Object.entries(insights.durabilityByOrigin).map(([origin, data]) => (
                    ...
                    <div className="flex gap-4 text-xs text-muted-foreground">
                      <span>
                        {t('repoCharts.durability.introduced', {
                          count: data.lines_introduced.toLocaleString(numberLocale),
                        })}
                      </span>
                      <span>
                        {t('repoCharts.durability.surviving', {
                          count: data.lines_surviving.toLocaleString(numberLocale),
                        })}
                      </span>
                      <span>
      ...
      {insights.activityTimeline && insights.activityTimeline.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t('repoCharts.weeklyActivity.subtitle')}</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border text-left text-muted-foreground">
                    <th className="pb-2 pr-4">{t('repoCharts.weeklyActivity.columns.week')}</th>
                    <th className="pb-2 pr-4">{t('repoCharts.weeklyActivity.columns.commits')}</th>
                    <th className="pb-2 pr-4">{t('repoCharts.weeklyActivity.columns.loc')}</th>
                    <th className="pb-2 pr-4">{t('repoCharts.weeklyActivity.columns.feature')}</th>
                    <th className="pb-2 pr-4">{t('repoCharts.weeklyActivity.columns.fix')}</th>
                    <th className="pb-2">{t('repoCharts.weeklyActivity.columns.aiPct')}</th>
                  </tr>
                </thead>
                <tbody>
                  {insights.activityTimeline.map((w) => {
                    ...
                    return (
                      <tr key={w.week_start} className="border-b border-border/50">
                        ...
                        <td className="py-2 pr-4">{w.lines_changed.toLocaleString(numberLocale)}</td>
                        <td className="py-2 pr-4">
                          {(((intent.FEATURE ?? 0) / totalIntent) * 100).toFixed(0)}%
                        </td>
                        <td className="py-2 pr-4">
                          {(((intent.FIX ?? 0) / totalIntent) * 100).toFixed(0)}%
                        </td>
                        <td className="py-2">
                          {(((origin.AI_ASSISTED ?? 0) / totalOrigin) * 100).toFixed(0)}%
                        </td>
                      </tr>
                    );
                  })}
      ...
      {aiImpact && aiImpact.length >= 2 && (
        <>
          <div className="pt-2">
            <h2 className="text-lg font-medium">{t('repoCharts.aiImpact.title')}</h2>
