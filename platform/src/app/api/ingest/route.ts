...
export async function POST(request: Request) {
  return withSpan("ingest", { "http.method": "POST", "http.route": "/api/ingest" }, async (parentSpan) => {
  // 1. Validate token
  ...
  const { repository, remote_url, window_days, github_user, active_users, metrics } =
    parsed.data;

  // Prefer version from header; fall back to body field for older CLIs
  const cli_version =
    request.headers.get("x-iris-cli-version") ?? parsed.data.cli_version ?? null;
