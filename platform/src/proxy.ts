...
// Pages that only make sense when email + password auth is enabled.
// When the feature is off, these redirect to /auth/signin so users land on
// the OAuth options instead of a dead form.
const PASSWORD_AUTH_PAGES = new Set([
  "/auth/signup",
...
export default async function proxy(request: NextRequest) {
  ...
  // Redirect password-auth-only pages to /auth/signin when the feature is off.
  // Must run before the early-return below, otherwise we'd let the page render
  // a dead form.
  if (PASSWORD_AUTH_PAGES.has(pathname) && !isFeatureEnabled("passwordAuth")) {
    const url = request.nextUrl.clone();
    url.pathname = "/auth/signin";
    url.search = "";
    const redirect = NextResponse.redirect(url);
    ensureLanguageCookie(request, redirect);
    return redirect;
  }
  ...
  if (tenantContext.tenant) {
    ...
    response.headers.set(
      "x-is-subdomain",
      tenantContext.isSubdomain.toString(),
    );
